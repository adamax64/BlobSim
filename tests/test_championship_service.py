...
class TestChampionshipService(unittest.TestCase):
    ...
    def test_end_season_if_over(self, mock_save_all_blobs, mock_get_all_by_league_order_by_id, mock_get_standings, mock_count_unconcluded):
        session = MagicMock(spec=Session)
        mock_standings = [
            StandingsDTO(blob_id=1, name="Blob 1", color="#111111", is_contract_ending=False, results=[], num_of_rounds=10, total_points=100),
            StandingsDTO(blob_id=2, name="Blob 2", color="#222222", is_contract_ending=False, results=[], num_of_rounds=10, total_points=90),
            StandingsDTO(blob_id=3, name="Blob 3", color="#333333", is_contract_ending=False, results=[], num_of_rounds=10, total_points=80),
            StandingsDTO(blob_id=4, name="Blob 4", color="#444444", is_contract_ending=False, results=[], num_of_rounds=10, total_points=70),
            StandingsDTO(blob_id=5, name="Blob 5", color="#555555", is_contract_ending=False, results=[], num_of_rounds=10, total_points=60),
            StandingsDTO(blob_id=6, name="Blob 6", color="#666666", is_contract_ending=False, results=[], num_of_rounds=10, total_points=50)
        ]
