...
from domain.league_service import (
    get_all_real_leagues,
    _correct_contract_of_inactive_leagues,
    _promote_blobs_to_leagues,
    _create_new_league_if_necessary,
    _promote_dropout_winner_if_possibble,
    _get_dropout_winner,
    _demote_blobs_to_dropout,
    _get_free_spaces,
    _get_blobs_by_standings_order,
    _get_most_recent_standings,
    _retire_blobs,
)
...
class TestLeagueService(unittest.TestCase):
    ...
    @patch("domain.league_service.transfers")
    @patch(
        "data.persistence.result_repository.get_most_recent_real_league_result_of_blob"
    )
    @patch("domain.league_service.get_standings")
    def test_promote_dropout_winner_if_possibble(
