...
from data.persistence.calendar_repository import (
    clear_calendar,
    get_calendar,
    get_next_unconcluded,
    save_all_calendar_records,
    save_calendar_event,
)
...
def recreate_calendar_for_next_season(session: Session, next_season: int):
    ...
    calendar.append(
        _create_calendar_record(next_season, 1, 0, None, EventType.CATCHUP_TRAINING)
    )
    calendar.append(
        _create_calendar_record(next_season, 1, 1, None, EventType.CATCHUP_TRAINING)
    )
    calendar.append(
        _create_calendar_record(next_season, 1, 2, None, EventType.CATCHUP_TRAINING)
    )
    calendar.append(
        _create_calendar_record(next_season, 1, 3, None, EventType.CATCHUP_TRAINING)
    )
...
def _get_event_id_by_date(session, date) -> int | None:
    event = get_event_by_date(session, date)
