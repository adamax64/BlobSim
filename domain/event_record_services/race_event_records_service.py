...
def get_endurance_event_records(
    ...
    event_records_by_competitors = {
        competitor.id: RaceEventRecordDto(blob=competitor, distance_records=[])
    ...
    current_tick = (
        ...
        else max((len(action.scores) for action in actions), default=0)
    )
    ...
    for tick in range(current_tick):
        for action in actions:
            if len(action.scores) > tick:
                ...
                previous_distance = (
                    competitor.distance_records[-1]
                    if len(competitor.distance_records) > 0
                    else 0
                )
                competitor.distance_records.append(
                    previous_distance + action.scores[tick]
                )
        if tick == current_tick - 2:
            sorted_competitors = sorted(
                event_records_by_competitors.values(),
                key=_endurance_sort_lambda(),
                reverse=True,
            )
...
def get_sprint_event_records(
    ...
    """
    event_records_by_competitors = {
        competitor.id: SprintEventRecordDto(blob=competitor, distance_records=[])
        for competitor in competitors
    }
    ...
    current_sorted = sorted(
        event_records_by_competitors.values(), key=_sprint_sort_lambda(), reverse=False
    )
