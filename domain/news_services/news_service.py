...
def add_blob_created_news(blob_name: str, session: Session):
    _create_news(NewsTypeDbo.BLOB_CREATED, [blob_name], session)
    delete_news_with_type(NewsTypeDbo.BLOB_IN_CREATION, session)


@transactional
def add_blob_in_creation_news(session: Session):
    _create_news(NewsTypeDbo.BLOB_IN_CREATION, [], session)


@transactional
def add_blob_terminated_news(blob_name: str, session: Session):
    _create_news(NewsTypeDbo.BLOB_TERMINATED, [blob_name], session)


@transactional
def add_event_starting_news(league_name: str, round: int, event_type: EventType, session: Session):
    _create_news(NewsTypeDbo.EVENT_STARTED, [league_name, str(round), str(event_type)], session)


@transactional
def add_ongoing_event_news(league_name: str, round: int, event_type: EventType, session: Session):
    _create_news(NewsTypeDbo.ONGOING_EVENT, [league_name, str(round), str(event_type)], session)
    delete_news_with_type(NewsTypeDbo.EVENT_STARTED, session)


@transactional
def add_event_ended_news(league_name: str, round: int, first: str, second: str, third: str, session: Session):
    _create_news(NewsTypeDbo.EVENT_ENDED, [league_name, str(round), first, second, third], session)
    delete_news_with_type(NewsTypeDbo.ONGOING_EVENT, session)


@transactional
def add_season_ended_news(league_name: str, winner: str, session: Session):
    _create_news(NewsTypeDbo.SEASON_ENDED, [league_name, winner], session)


@transactional
def add_rookie_of_the_year_news(winner: str, session: Session):
    _create_news(NewsTypeDbo.ROOKIE_OF_THE_YEAR, [winner], session)
...
def add_new_grandmaster_news(grandmaster: str, session: Session):
    _create_news(NewsTypeDbo.NEW_GRANDMASTER, [grandmaster], session)


def _create_news(news_type: NewsTypeDbo, data: list[str], session: Session):
