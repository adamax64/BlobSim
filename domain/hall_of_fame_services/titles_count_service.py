...
def get_titles_count(session) -> TitlesCountSummaryDto:
    ...
    return TitlesCountSummaryDto(
        ...
        championships=_get_champion_counts(
            session, current_season, grandmaster_id, blobs
        ),
        top_wins=_get_top_win_counts(session, current_season, grandmaster_id, blobs),
        top_podiums=_get_top_podium_counts(
            session, current_season, grandmaster_id, blobs
        ),
        season_victories=_get_lower_victory_counts(
            session, current_season, grandmaster_id, blobs
        ),
        lower_wins=_get_lower_win_counts(
            session, current_season, grandmaster_id, blobs
        ),
        lower_podiums=_get_lower_podium_counts(
            session, current_season, grandmaster_id, blobs
        ),
    )


def _get_grandmaster_counts(
    ...
    standings_by_league = get_standings_by_league(
        session, list(grandmaster_counts.keys()), current_season
    )
...
def _get_champion_counts(
    session, current_season: int, grandmaster_id: int, blobs
) -> list[TitleCountDto]:
    ...
    standings_by_league = get_standings_by_league(
        session, list(champion_counts.keys()), current_season
    )
...
def _get_top_win_counts(
    session, current_season: int, grandmaster_id: int, blobs
) -> list[TitleCountDto]:
    blobs_sorted = sorted(blobs, key=lambda b: b.gold_trophies, reverse=True)
    standings_by_league = get_standings_by_league(session, blobs_sorted, current_season)
    ...
    return [
        ...
        if blob.gold_trophies > 0
    ]


def _get_top_podium_counts(
    session, current_season: int, grandmaster_id: int, blobs
) -> list[TitleCountDto]:
    blobs_sorted = sorted(
        blobs,
        key=lambda b: b.bronze_trophies + b.silver_trophies + b.gold_trophies,
        reverse=True,
    )
    standings_by_league = get_standings_by_league(session, blobs_sorted, current_season)
    ...
    return [
        ...
        for blob in blobs_sorted
        if (blob.bronze_trophies + blob.silver_trophies + blob.gold_trophies) > 0
    ]


def _get_lower_victory_counts(
    session, current_season: int, grandmaster_id: int, blobs
) -> list[TitleCountDto]:
    ...
    return [
        ...
        for blob in blobs_sorted
        if blob.season_victories > 0
    ]


def _get_lower_win_counts(
    session, current_season: int, grandmaster_id: int, blobs
) -> list[TitleCountDto]:
    ...
    return [
        ...
        for blob in blobs_sorted
        if blob.gold_medals > 0
    ]


def _get_lower_podium_counts(
    session, current_season: int, grandmaster_id: int, blobs
) -> list[TitleCountDto]:
    blobs_sorted = sorted(
        blobs,
        key=lambda b: b.bronze_medals + b.silver_medals + b.gold_medals,
        reverse=True,
    )
    standings_by_league = get_standings_by_league(session, blobs_sorted, current_season)
    ...
    return [
        ...
        for blob in blobs_sorted
        if (blob.bronze_medals + blob.silver_medals + blob.gold_medals) > 0
    ]
