...
@router.get("/can_progress")
async def can_progress(_=Depends(require_auth)) -> bool:
    try:
        return not (
            sim_data_service.is_unconcluded_event_today()
            or sim_data_service.is_blob_created()
        )
    except Exception as e:
        traceback.print_exc()
        raise HTTPException(status_code=500, detail=f"{e.with_traceback(None)}")
...
def progress(_=Depends(require_auth)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Error while updating blobs: {e.with_traceback(None)}",
        )
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Error while updating simulation progress: {e.with_traceback(None)}",
        )
