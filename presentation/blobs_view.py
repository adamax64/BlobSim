...
from presentation.utils import capture_keypress, format_sim_time_short
...
def show_blobs_view(live: Live):
    ...
    def render():
        table = Table(title='Select a blob to view options or press Escape to return', min_width=50)
        ...
        for i, blob in enumerate(blobs):
            table.add_row(
                f'[cyan]{str(blob.name)}[/cyan]' if i == current_row else str(blob.name),
                str(format_sim_time_short(blob.born)),
