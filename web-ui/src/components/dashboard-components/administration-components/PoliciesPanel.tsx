...
import { Box, Tooltip, Typography, Paper, Skeleton } from '@mui/material';
...
export const PoliciesPanel = () => {
  ...
  const { policies: policyData, refreshPolicies, policiesLoading } = usePolicies();
  ...
  return (
    <Box display="flex" flexDirection="row" gap={1} alignItems="center">
      {policiesLoading
        ? policies.map((p) => <Skeleton key={p.type} variant="rounded" width={50} height={50} />)
        : policies.map((p) => (
            <Tooltip
              key={p.type}
              title={
